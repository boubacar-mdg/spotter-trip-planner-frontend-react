...
const LogSheetSvg = ({ logData, sortedLogs, onDateChange }: { logData: EldLog, sortedLogs: any, onDateChange:any }) => {
  ...
  const renderSvgGrid = () => {
    ...
    for (let i = 1; i < statusSections.length; i++) {
      ...
      gridLines.push(
        <line key={`h-line-${i}`} x1={50} y1={y} x2={width - 50} y2={y} stroke="#000000" strokeWidth={1} />
      );
    }
    ...
    for (let i = 0; i <= 24; i++) {
      ...
      gridLines.push(
        <line key={`v-line-${i}`} x1={x} y1={10} x2={x} y2={height - 110} stroke="#000000" strokeWidth={1} />
      );
      
      // Hour labels
      if (i < 24) {
        gridLines.push(
          <text key={`hour-${i}`} x={x + timeColWidth / 2} y={height - 95} textAnchor="middle" fontSize="10px">{`${i}:00`}</text>
        );
      }
    }
    ...
    Object.entries(statusCodes).forEach(([code, status]) => {
      const y = status.y;
      const nextY = Object.values(statusCodes).find(s => s.y > y)?.y || height - 110;
      const midY = y + (nextY - y) / 2;
      
      gridLines.push(
        <text key={`status-${code}`} x={45} y={midY} textAnchor="end" fontSize="10px">{status.label}</text>
      );
    });
    ...
    return (
      <>
        {/* Border */}
        <rect x={50} y={10} width={width - 100} height={height - 120} stroke="#000000" strokeWidth={1} fill="none" />
        {gridLines}
      </>
    );
  };
  ...
  const renderSvgEvents = () => {
    ...
    for (let i = 0; i < sortedEvents.length - 1; i++) {
      ...
      const currentX = 50 + ((currentTime[0] * 60 + currentTime[1]) / 60) * timeColWidth;
  ...
  const renderSvgHeader = () => {
    ...
    return (
      <>
        <text x={50} y={height - 80} fontSize="12px">{`Driver: ${logData.driver_name || 'N/A'}`}</text>
        <text x={50} y={height - 65} fontSize="12px">{`Carrier: ${logData.carrier || 'N/A'}`}</text>
        <text x={50} y={height - 50} fontSize="12px">{`Truck #: ${logData.truck_number || 'N/A'}`}</text>
        <text x={50} y={height - 35} fontSize="12px">{`Trailer #: ${logData.trailer_numbers || 'N/A'}`}</text>
        
        <text x={width - 50} y={height - 80} textAnchor="end" fontSize="12px">{`Date: ${new Date(logData.date).toLocaleDateString()}`}</text>
        <text x={width - 50} y={height - 65} textAnchor="end" fontSize="12px">{`Shipping Doc: ${logData.shipping_doc || 'N/A'}`}</text>
      </>
    );
  };
  ...
  const renderSvgSummary = () => {
    ...
    return (
      <>
        <text x={width - 200} y={height - 35} fontSize="12px">Hours Summary:</text>
        <text x={width - 200} y={height - 20} fontSize="12px">{`Drive: ${summary.driving}h`}</text>
        <text x={width - 120} y={height - 20} fontSize="12px">{`On Duty: ${summary.on_duty_not_driving}h`}</text>
        <text x={width - 200} y={height - 5} fontSize="12px">{`SB: ${summary.sleeper_berth}h`}</text>
        <text x={width - 120} y={height - 5} fontSize="12px">{`Off Duty: ${summary.off_duty}h`}</text>
      </>
    );
  };
  ...
  return (
    <div className="log-sheet">
      <div className="flex justify-between items-center">
        ...
        <CustomSelect options={sortedLogs.map((log:any, index:number) => {console.log(log); return {key: index, name: new Date(log.date).toLocaleDateString()} })} onChange={onDateChange} />
        </div>
      ...
      <div className="log-sheet-info bg-gray-50 p-4 rounded border border-gray-200 mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <p><strong>Driver:</strong> {logData?.driver_name || 'N/A'}</p>
          <p><strong>Carrier:</strong> {logData?.carrier || 'N/A'}</p>
          <p><strong>Truck #:</strong> {logData?.truck_number || 'N/A'}</p>
          <p><strong>Trailer #:</strong> {logData?.trailer_numbers || 'N/A'}</p>
        </div>
      </div>
      ...
      {logData?.hours_summary && (
        <div className="hours-summary bg-[#008080]/10 p-4 rounded border-0} mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-2 text-center rounded" style={{ backgroundColor: statusCodes['D'].color, border: '1px solid #ccc' }}>
              <p className="font-semibold">{statusCodes['D'].label}</p>
              <p className="text-xl">{logData.hours_summary.driving}h</p>
            </div>
            <div className="p-2 text-center rounded" style={{ backgroundColor: statusCodes['ON'].color, border: '1px solid #ccc' }}>
              <p className="font-semibold">{statusCodes['ON'].label}</p>
              <p className="text-xl">{logData.hours_summary.on_duty_not_driving}h</p>
            </div>
            <div className="p-2 text-center rounded" style={{ backgroundColor: statusCodes['SB'].color, border: '1px solid #ccc' }}>
              <p className="font-semibold">{statusCodes['SB'].label}</p>
              <p className="text-xl">{logData.hours_summary.sleeper_berth}h</p>
            </div>
            <div className="p-2 text-center rounded" style={{ backgroundColor: statusCodes['OFF'].color, border: '1px solid #ccc' }}>
      ...
      <div className="events-section">
        ...
        <div className="overflow-x-auto">
          <table className="events-table w-full border-collapse mb-4">
            ...
            <tbody>
              {logData?.events && sortEvents(logData.events).map((event, index) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  <td className="border p-2">{event.time}</td>
                  <td className="border p-2">
                    <span 
                      className="inline-block w-3 h-3 rounded-full mr-2"
                      style={{ backgroundColor: statusCodes[event.status].color, border: '1px solid #000' }}
                    ></span>
